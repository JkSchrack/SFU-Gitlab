...
{
  intarr_t* NewVar = malloc(sizeof(intarr_t)); // Allocating for new variable of type intarr_t
  NewVar->data = malloc(sizeof(int)*len); // Allocating for field which is array of type int
  NewVar->len = len; // 
  if (NewVar == NULL)
  {
    return NULL;
...
intarr_t* intarr_copy( const intarr_t* ia )
{
  ...
  else
  {
    ...
    duplicate->data = ia->data;
    duplicate->len = ia->len;

    unsigned int bytes = duplicate->len * sizeof(int);
    duplicate->data = malloc(bytes);

    memcpy(duplicate->data, ia->data, bytes);
...
intarr_result_t intarr_push( intarr_t* ia, int val )
{
  ...
  {
    int newlen = ia->len + 1;
    intarr_t* temp = realloc(ia->data, newlen * sizeof(int));
    if (temp == NULL)
    {
    ...
    {
      ia->data = realloc(temp, newlen * sizeof(int));
      ia->len = newlen;
...
intarr_result_t intarr_pop( intarr_t* ia, int* i )
{
  ...
  if (ia->len != 0 && i != NULL)
  {
    ...
    ia->data = realloc(ia->data, size * sizeof(int));
    if (ia->data != NULL)
    {
      *i = ia->data[last];
      ia->len = last;
       ...
       unsigned int last )
{
  ...
  if( last > ia->len || first > last)
  {
    return NULL;
  }
  // first/last are out of bounds
  ...
  unsigned int bytes = new->len * sizeof(int);
  new->data = malloc(bytes);
  memcpy(new->data, ia->data, bytes);
  // deep copy
  ...
  intarr_t* temp = realloc(new->data, newlen * sizeof(int));
  if (temp == NULL)
  {
    return NULL;
  }
  ...
  {
    new->len = newlen;
    new->data = realloc(temp, newlen * sizeof(int));
    return new;
  }
}
